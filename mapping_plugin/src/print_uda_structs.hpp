...
{
template <typename T> inline std::string print_typed_buffer(T* data, int max_size)
{
...
inline std::string print_request_data(REQUEST_DATA* request_data)
{
    ...
    for (int i = 0; i < request_data->nameValueList.pairCount; i++) {
        ss << i << ", " << request_data->nameValueList.nameValue[i].pair << ", "
           << request_data->nameValueList.nameValue[i].name << ", " << request_data->nameValueList.nameValue[i].value
           << std::endl;
    }
