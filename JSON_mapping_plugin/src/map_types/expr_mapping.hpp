...
template <typename T> int ExprMapping::eval_expr(const MapArguments& arguments) const
{
    ...
    for (const auto& [key, json_name] : m_parameters) {
        ...
        T* raw_data = reinterpret_cast<T*>(arguments.m_interface->data_block->data);
        size_t data_size = arguments.m_interface->data_block->data_n;
    ...
    if (vector_expr) {
        imas_json_plugin::uda_helpers::set_return_data_array_vec(
            arguments.m_interface->data_block,
            result
        );
    } else {
